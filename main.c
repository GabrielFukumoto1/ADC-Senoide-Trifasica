...
#define PI 3.14159265358979323846
                                                    ...
                                                    //
__interrupt void funcao_seno(void);                 // declaracao da funcao de interrupcao
void Adc_Config(void);                              // declaracao de funcao
void freq (float);                                  //
void dutty (float);                                 //
                                                    //
float GrafSin1[100];                                 // (inteiro sem sinal de 16 bits) feito com typedef -- buffer com 100 posicoes para guardar valores do ADC
float GrafSin2[100];
float GrafSin3[100];
...
int main(void)
{
    ...
    InitPieVectTable();                             //  trabalha junto com initpiectrl  (configura priorirdades de interrupcoes)
                                                    //
    EALLOW;                                         //
    PieVectTable.ADCINT1 = &funcao_seno;                // interrupcao associada ao endereço da função adc_isr
    EDIS;                                           //
                                                    //
...
__interrupt void  funcao_seno(void)                             // função usada apos a interrupcao do ADC
{
        ...
        GrafSin1[1] = sin(2*PI* contador/30000);
        GrafSin2[1] = sin(2*PI* contador/30000 + (defasagem));
        GrafSin3[1] = sin(2*PI* contador/30000 + (2*defasagem));
